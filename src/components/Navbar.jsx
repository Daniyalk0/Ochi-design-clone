...
function Navbar() {
  ...
  return (
    <div
      className={`fixed z-[999] w-full py-6 px-20 font-[NeueMontreal-Regular.ttf] flex justify-between  items-center backdrop-filter backdrop-blur-md ${
        showNavbar ? "tablet:-translate-y-0" : "tablet:-translate-y-20"
      } transition-all duration-300 ease-in-out     tablet:px-6 tablet:py-4 `}
    >
      <div className="tablet:relative tablet:z-[999]">
        <svg
          width="72"
          height="30"
          viewBox="0 0 72 30"
      ...
      <div
        className="absolute tablet:z-[1000] tablet:left-[88vw] hidden tablet:flex "
        onClick={() => {
          setOpen((prev) => prev), console.log(isOpen);
        }}
      >
        <Hamburger
          toggled={isOpen}
      ...
      <div
        className={`links flex gap-10 tablet:flex-col tablet:bg-red-400 w-1/2 tablet:min-h-screen  tablet:w-full   ${
          isOpen ? "top-0" : "top-[-100vh]"
        } transition-all duration-200 tablet:font-custom tablet:absolute tablet:left-0  tablet:px-6 tablet:py-20`}
      >
        <div className="hidden tablet:flex bg-black w-full h-[2px]"></div>
          ...
          (links, index) => (
            <a
              ...
              className={`text-sm font-light ${
                index === 4 && "ml-36 tablet:ml-0"
              } tablet:text-[10vw] tablet:uppercase tablet:py-1`}
            >
              {links}
            </a>
          )
